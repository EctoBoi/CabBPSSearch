...
function inject() {
  ...
  if (!document.getElementById("search-button")) {
    if (
      document.getElementsByClassName(
        "heading heading-largest has-subHeading"
      )[0]
    ) {
      if (document.getElementsByClassName("text-red")[0]) {
        let productName = document
          .getElementsByClassName("text-red")[0]
          .innerHTML.replace(/[^a-zA-Z0-9 -]/g, "");
        document
          .getElementsByClassName("results")[0]
          .append(createSearchButton(createBPSURL(productName)));
      } else {
        let productName = document
          .getElementsByClassName("text-blue")[0]
          .innerHTML.replace(/[^a-zA-Z0-9 -]/g, "");
        document
          .getElementsByClassName("results")[0]
          .append(createSearchButton(createBPSURL(productName)));
  ...
  if (
    typeof document.getElementsByClassName("product-article")[0] !== "undefined"
  )
    if (!document.getElementById("search-button")) {
      //Font
      ...
      if (bpssku === undefined) {
        ...
        document
          .getElementsByClassName("product-article")[0]
          .appendChild(copyDiv);
      }
      article.getElementsByTagName("b")[0].innerHTML =
        article.getElementsByTagName("b")[0].innerHTML + "&nbsp;";
...
function createBPSURL(productName) {
  return (
    "https://www.basspro.com/shop/en/SearchDisplay?categoryId=&searchTerm=" +
